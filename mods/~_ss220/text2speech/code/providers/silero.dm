...
/datum/tts_provider/silero/request(text, datum/tts_seed/silero/seed, datum/callback/proc_callback)
	...
	req_body["api_token"] = config.tts_token_silero
	...
	SShttp.create_async_request(RUSTG_HTTP_METHOD_POST, config.tts_api_url_silero , json_encode(req_body), list("content-type" = "application/json"), proc_callback)
