...
/spell/area_teleport/choose_targets()
	...
	if(!randomise_selection)
		thearea = input("Area to teleport to", "Teleport") as null|anything in GLOB.wizteleportlocs
		if(!thearea)
			return
	else
		thearea = pick(GLOB.wizteleportlocs)
	return list(GLOB.wizteleportlocs[thearea])
