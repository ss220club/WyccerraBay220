...
/obj/item/device/assembly/infra/New()
	...
	beams = list()
	seen_turfs = list()
	proximity_trigger = new(src, TYPE_PROC_REF(/obj/item/device/assembly/infra, on_beam_entered), TYPE_PROC_REF(/obj/item/device/assembly/infra, on_visibility_change), world.view, GLOB.PROXIMITY_EXCLUDE_HOLDER_TURF)
