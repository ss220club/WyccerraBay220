...
//Regular jobs
	//Command (Blue)
		...
		jobs += "<tr align='center' bgcolor='ccccff'><th colspan='[length(SSjobs.titles_by_department(GLOB.COM))]'><a href='?src=\ref[src];jobban3=commanddept;jobban4=\ref[M]'>Command Positions</a></th></tr><tr align='center'>"
		for(var/jobPos in SSjobs.titles_by_department(GLOB.COM))
			if(!jobPos)	continue
	...
	//Command Support (Sky Blue)
		jobs += "<table cellpadding='1' cellspacing='0' width='100%'>"
		jobs += "<tr bgcolor='87ceeb'><th colspan='[length(SSjobs.titles_by_department(GLOB.SPT))]'><a href='?src=\ref[src];jobban3=supportdept;jobban4=\ref[M]'>Command Support Positions</a></th></tr><tr align='center'>"
		for(var/jobPos in SSjobs.titles_by_department(GLOB.SPT))
			if(!jobPos)	continue
			var/datum/job/job = SSjobs.get_by_title(jobPos)
			if(!job) continue
	...
	//Security (Red)
		counter = 0
		jobs += "<table cellpadding='1' cellspacing='0' width='100%'>"
		jobs += "<tr bgcolor='ffddf0'><th colspan='[length(SSjobs.titles_by_department(GLOB.SEC))]'><a href='?src=\ref[src];jobban3=securitydept;jobban4=\ref[M]'>Security Positions</a></th></tr><tr align='center'>"
		for(var/jobPos in SSjobs.titles_by_department(GLOB.SEC))
			if(!jobPos)	continue
			var/datum/job/job = SSjobs.get_by_title(jobPos)
			if(!job) continue
	...
	//Engineering (Yellow)
		counter = 0
		jobs += "<table cellpadding='1' cellspacing='0' width='100%'>"
		jobs += "<tr bgcolor='fff5cc'><th colspan='[length(SSjobs.titles_by_department(GLOB.ENG))]'><a href='?src=\ref[src];jobban3=engineeringdept;jobban4=\ref[M]'>Engineering Positions</a></th></tr><tr align='center'>"
		for(var/jobPos in SSjobs.titles_by_department(GLOB.ENG))
			if(!jobPos)	continue
			var/datum/job/job = SSjobs.get_by_title(jobPos)
			if(!job) continue
	...
	//Medical (White)
		counter = 0
		jobs += "<table cellpadding='1' cellspacing='0' width='100%'>"
		jobs += "<tr bgcolor='ffeef0'><th colspan='[length(SSjobs.titles_by_department(GLOB.MED))]'><a href='?src=\ref[src];jobban3=medicaldept;jobban4=\ref[M]'>Medical Positions</a></th></tr><tr align='center'>"
		for(var/jobPos in SSjobs.titles_by_department(GLOB.MED))
			if(!jobPos)	continue
			var/datum/job/job = SSjobs.get_by_title(jobPos)
			if(!job) continue
	...
	//Science (Purple)
		counter = 0
		jobs += "<table cellpadding='1' cellspacing='0' width='100%'>"
		jobs += "<tr bgcolor='e79fff'><th colspan='[length(SSjobs.titles_by_department(GLOB.SCI))]'><a href='?src=\ref[src];jobban3=sciencedept;jobban4=\ref[M]'>Science Positions</a></th></tr><tr align='center'>"
		for(var/jobPos in SSjobs.titles_by_department(GLOB.SCI))
			if(!jobPos)	continue
			var/datum/job/job = SSjobs.get_by_title(jobPos)
			if(!job) continue
	...
	//Exploration (Pale Purple)
		jobs += "<table cellpadding='1' cellspacing='0' width='100%'>"
		jobs += "<tr bgcolor='b784a7'><th colspan='[length(SSjobs.titles_by_department(GLOB.EXP))]'><a href='?src=\ref[src];jobban3=explorationdept;jobban4=\ref[M]'>Exploration Positions</a></th></tr><tr align='center'>"
		for(var/jobPos in SSjobs.titles_by_department(GLOB.EXP))
			if(!jobPos)	continue
			var/datum/job/job = SSjobs.get_by_title(jobPos)
			if(!job) continue
	...
	//Service (Tea Green)
		jobs += "<table cellpadding='1' cellspacing='0' width='100%'>"
		jobs += "<tr bgcolor='d0f0c0'><th colspan='[length(SSjobs.titles_by_department(GLOB.SRV))]'><a href='?src=\ref[src];jobban3=servicedept;jobban4=\ref[M]'>Service Positions</a></th></tr><tr align='center'>"
		for(var/jobPos in SSjobs.titles_by_department(GLOB.SRV))
			if(!jobPos)	continue
			var/datum/job/job = SSjobs.get_by_title(jobPos)
			if(!job) continue
	...
	//Supply (Khaki)
		jobs += "<table cellpadding='1' cellspacing='0' width='100%'>"
		jobs += "<tr bgcolor='f0e68c'><th colspan='[length(SSjobs.titles_by_department(GLOB.SUP))]'><a href='?src=\ref[src];jobban3=supplydept;jobban4=\ref[M]'>Supply Positions</a></th></tr><tr align='center'>"
		for(var/jobPos in SSjobs.titles_by_department(GLOB.SUP))
			if(!jobPos)	continue
			var/datum/job/job = SSjobs.get_by_title(jobPos)
			if(!job) continue
	...
	//Civilian (Grey)
		counter = 0
		jobs += "<table cellpadding='1' cellspacing='0' width='100%'>"
		jobs += "<tr bgcolor='dddddd'><th colspan='[length(SSjobs.titles_by_department(GLOB.CIV))]'><a href='?src=\ref[src];jobban3=civiliandept;jobban4=\ref[M]'>Civilian Positions</a></th></tr><tr align='center'>"
		for(var/jobPos in SSjobs.titles_by_department(GLOB.CIV))
			if(!jobPos)	continue
			var/datum/job/job = SSjobs.get_by_title(jobPos)
			if(!job) continue
	...
	//Non-Human (Green)
		counter = 0
		jobs += "<table cellpadding='1' cellspacing='0' width='100%'>"
		jobs += "<tr bgcolor='ccffcc'><th colspan='[length(SSjobs.titles_by_department(GLOB.MSC))+1]'><a href='?src=\ref[src];jobban3=nonhumandept;jobban4=\ref[M]'>Non-human Positions</a></th></tr><tr align='center'>"
		for(var/jobPos in SSjobs.titles_by_department(GLOB.MSC))
			if(!jobPos)	continue
			var/datum/job/job = SSjobs.get_by_title(jobPos)
			if(!job) continue
