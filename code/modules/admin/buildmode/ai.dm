...
/datum/build_mode/ai/OnClick(atom/A, list/pa)
	...
	if (pa[RIGHT_CLICK])
		...
		if (isliving(A))
			...
			if (pa[ALT_CLICK] && pa[SHIFT_CLICK])
				if (!ai_type)
		...
		if (isturf(T)) // Move or reposition.
			...
			for(var/mob/living/unit in selected_mobs)
				...
				if (unit.get_AI_stance() == STANCE_SLEEP)
					...
					AI.give_destination(T, 0, pa[SHIFT_CLICK]) // If shift is held, the mobs will not stop moving to attack a visible enemy.
					told++
	...
	if (pa[MIDDLE_CLICK])
		if(isliving(A)) // Follow or attack.
			...
			for(var/mob/living/unit in selected_mobs)
				...
				if (!AI)
					return
				if(L.IIsAlly(unit) || !AI.hostile || pa[SHIFT_CLICK])
					AI.set_follow(L)
					j++
				else
					AI.give_target(L)
