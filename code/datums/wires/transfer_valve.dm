/datum/wires/transfer_valve
	...
	wire_count = 5
	descriptions = list(
		new /datum/wire_description(GLOB.TTV_WIRE_TOGGLEVALVE, "This wire is connected to the valve latch controllers.", SKILL_EXPERIENCED),
		new /datum/wire_description(GLOB.TTV_WIRE_GASRELEASE, "This wire seems to be connected directly to the tank latch.", SKILL_TRAINED),
		new /datum/wire_description(GLOB.TTV_WIRE_DISARM, "This wire seems to be connected to the internal structural controller.", SKILL_TRAINED),
		new /datum/wire_description(GLOB.TTV_WIRE_DEVICECHANGE, "This wire seems to be connected to the attached mechanism.", SKILL_TRAINED),
		new /datum/wire_description(GLOB.TTV_WIRE_RNG, "This red wire seems to go over every little nook and cranny of the bomb.")
	)
...
/datum/wires/transfer_valve/UpdatePulsed(index)
	...
	switch(index)
		if (GLOB.TTV_WIRE_TOGGLEVALVE)
			T.toggle_valve()
		if (GLOB.TTV_WIRE_GASRELEASE)
			T.split_gases()
		if (GLOB.TTV_WIRE_RNG)
			T.toggle_valve()
		if (GLOB.TTV_WIRE_DEVICECHANGE)
