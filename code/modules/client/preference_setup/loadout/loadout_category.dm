...
/datum/loadout_category/New(category_name)
	src.category_name = category_name

/datum/loadout_category/proc/add_gear(datum/gear/gear_to_add)
	ASSERT(istype(gear_to_add))
	gear_items[gear_to_add.display_name] = gear_to_add

/datum/loadout_category/proc/sort_gear()
	gear_items = sortAssoc(gear_items)

/// Returns copy of gear `gear_items`
/datum/loadout_category/proc/get_gear_items()
	if(!gear_items)
