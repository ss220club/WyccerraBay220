...
/datum/ticket/New(datum/client_lite/owner)
	...
	addtimer(CALLBACK(src, PROC_REF(timeoutcheck)), 5 MINUTES, GLOB.TIMER_STOPPABLE)
...
/datum/ticket/proc/timeoutcheck()
	if(status == TICKET_OPEN)
		...
		addtimer(CALLBACK(src, PROC_REF(timeoutchecklast)), 5 MINUTES, GLOB.TIMER_STOPPABLE)
...
/datum/ticket/proc/timeoutchecktaken()
	if (status == TICKET_ASSIGNED)
		if (is_active())
			addtimer(CALLBACK(src, PROC_REF(timeoutchecktaken)), 30 MINUTES, GLOB.TIMER_STOPPABLE)
			return
